...
public class ECmc2 extends DetectorMonitor {
    ...
    public void createMAIN(int st) {
        ...
        case 0:  
            ...
            dg.addDataSet(makeH2("h2c",50,0,4,50,0,4,"","E #gamma 1 / E #gamma 2 (PCAL)","E #gamma 1 / E #gamma 2 (ECIN)"),n++);
            dg.addDataSet(makeH2("h2cc",50,0,4,50,0,4,"","E #gamma 1 / E #gamma 2 (PCAL)","E #gamma 1 / E #gamma 2 (ECIN)"),n++);
    ...
    public void createEFFICIENCY(int st) {
        ...
        case 0:          
            ...
            dg.addDataSet(makeH1("h11",50,0,5.2,"","Opening Angle (deg)","Counts",1,4),n++);
            dg.addDataSet(makeH1("h12",50,0,5.2,"n>0","Opening Angle (deg)","Efficiency",1,4),n++);
            dg.addDataSet(makeH1("h13",50,0,5.2,"n>1","Opening Angle (deg)","Efficiency",1,3),n++);
            dg.addDataSet(makeH1("h14",50,0,5.2,"n==2","Opening Angle (deg)","Efficiency",1,2),n++);
            dg.addDataSet(makeH1("h15",50,0,5.2,"n>=2","Opening Angle (deg)","Efficiency",1,5),n++);
    	}
        this.getDataGroup().add(dg,0,st,k,run); 
    }
    ...
    public void processEvent(DataEvent de) {
		...
		DataGroup dg1 = this.getDataGroup().getItem(0,0,getDetectorTabNames().indexOf("EFFICIENCY"),run);
		
		boolean goodev = eb.readMC(de) && eb.pmc.size()==2;
		float      opa =(float) ((goodev) ? eb.pmc.get(0).cosTheta(eb.pmc.get(1)):0);
		
		int npp=0,indx=-1; float pthresh=1f, etot=0f;
        ...
        if (goodev) {
        	...
        	dg1.getH1F("h11").fill(opa);
        	npp=0;indx=-1;
        	if (np.size()>0) {
        		dg1.getH1F("h12").fill(opa);
...
//        				System.out.println(npp+" "+indx+" "+ep);
        	...
        	dg1.getH2F("hnpp").fill(opa,npp);
        	if (npp>1)  dg1.getH1F("h13").fill(opa);
        	if (npp==2) dg1.getH1F("h14").fill(opa);
        	if (npp>=2) dg1.getH1F("h15").fill(opa);
        	if (npp>=2) {
...
//        analyze();         
    ...
    public void geteff() {
		...
		DataGroup dg0 = this.getDataGroup().getItem(0,0,getDetectorTabNames().indexOf("MAIN"),run);
		DataGroup dg1 = this.getDataGroup().getItem(0,0,getDetectorTabNames().indexOf("EFFICIENCY"),run);
		dg1.getH1F("h12").divide(dg1.getH1F("h12"),dg1.getH1F("h11"));
		dg1.getH1F("h13").divide(dg1.getH1F("h13"),dg1.getH1F("h11"));
		dg1.getH1F("h14").divide(dg1.getH1F("h14"),dg1.getH1F("h11"));
		dg1.getH1F("h15").divide(dg1.getH1F("h15"),dg1.getH1F("h11"));
		dg0.getH1F("h1gx").add(dg0.getH2F("h2g").projectionX());
		dg0.getH1F("h1gy").add(dg0.getH2F("h2g").projectionY());		
    }
