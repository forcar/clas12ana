...
public class ECperf extends DetectorMonitor {
	...
	public boolean goodE,goodP,goodPROT,goodPBAR,goodPIP,goodPIM,goodNEUT,goodNEUT2,goodPHOT,goodPHOT2,goodPI0;
	public int nE,nP,nPROT,nPBAR,nPIP,nPIM,nNEUT,nNEUT2,nPHOT,nPHOT2;
...
//	public float pi0_mass, pi0_mom, pi0_e, pi0_the, pi0_phi, pi0_open, pi0_cx, pi0_cy;
	...
	public List<Particle> phot2_ecal = new ArrayList<Particle>();
	public List<Particle> neut_ecal = new ArrayList<Particle>();	
	public List<Particle> neut2_ecal = new ArrayList<Particle>();	
...
//    	createECkin(6); 8/29/2022
    ...
    public void processEvent(DataEvent event) {
    	...
    	e_ecal    = ev.getPART(11,0.100);    nE = e_ecal.size();
    	p_ecal    = ev.getPART(12,0.100);    nP = p_ecal.size();
    	pim_ecal  = ev.getPART(212,0.005); nPIM = pim_ecal.size(); 
    	pip_ecal  = ev.getPART(211,0.200); nPIP = pip_ecal.size();
 ...
 //  	    System.out.println(e_sect+" "+getElecTriggerSector(0)+" "+getElecTriggerSector(1)+" "+getElecTriggerSector(2));
    	...
    	prot_ecal  = ev.getPART(2212,0.200);  nPROT = prot_ecal.size();
    	pbar_ecal  = ev.getPART(2213,0.200);  nPBAR = pbar_ecal.size();
    	neut_ecal  = ev.getPART(2112,0.001);  nNEUT = neut_ecal.size(); 
    	neut2_ecal = ev.getPART(2112,0.001); nNEUT2 = neut2_ecal.size(); 
    	phot_ecal  = ev.getPART(22,0.001);    nPHOT = phot_ecal.size();    	
    	phot2_ecal = ev.getPART(22,0.500);   nPHOT2 = phot2_ecal.size();
    ...
    void filterEvent() {
	    ...
	    if(goodP) makePOSI();
	    if(goodPHOT) {
	    	ecphot      = ev.getECALParticles(phot_ecal); //used in fillECnm*	    	
	    	ecphot_filt = ev.filterECALClusters(ev.getECALClusters(phot_ecal),2,22); //used in fillECelec for e-g residuals
	    }
	    if(goodPIM) ecpim_filt = ev.filterECALClusters(ev.getECALClusters(pim_ecal),4,212);
    } 
	...
	public void fillHists(){
	    ...
	    if(goodNEUT2 && goodPHOT2) fillECneut2();	    
...
//			dg.addDataSet(makeH2(tab+"-4-",tag,70,0,EB/6,70,0,EB/5, "","PC (GeV)","EC (GeV)"),is-1+18);	
    	...
    	case 4:  
    		...
    		for(int is=1;is<7;is++){   
    			tag = is+"-"+st+"-"+k+"-"+run;
    		    dg.addDataSet(makeH2(tab+"-1-",tag,60,thmin,thmax,60,pmin,pmax,"SECTOR "+is,"#theta neg (deg)","p (GeV)"),is-1);
    		    dg.addDataSet(makeH2(tab+"-2-",tag,60,thmin,thmax,60,pmin,pmax,"",          "#theta e-  (deg)","p (GeV)"),is-1+6);
    		    dg.addDataSet(makeH2(tab+"-3-",tag,60,thmin,thmax,60,pmin,pmax,"",          "#theta pi- (deg)","p (GeV)"),is-1+12);
    		}
    	break;	
    ...
    public void createECneut(int st) {
    	...
    	break;
    case 2:
    	dg = new DataGroup(3,3); n=0;
    	tag = st+"-"+k+"-"+run;   
    	dg.addDataSet(makeH2(tab+"-"+n+"-",tag,150,20,80,50,0,200,"pid=2112","pcal time (ns)","Edep (MeV)"),n);n++;
    	dg.addDataSet(makeH2(tab+"-"+n+"-",tag,150,20,80,50,0,200,"pid=2112","ecin time (ns)","Edep (MeV)"),n);n++;
    	dg.addDataSet(makeH2(tab+"-"+n+"-",tag,150,20,80,50,0,200,"pid=2112","ecou time (ns)","Edep (MeV)"),n);n++;
    	}
    ...
    public boolean makeELEC(){
        ...
        List<Particle> elecECAL = ev.getECAL((int)epart.getProperty("pindex")); //list contains particle objects mapped to each associated calorimeter cluster (pcal,ecin,ecou)
...
//    	Boolean good_trig = elecECAL.get(0).getProperty("sector")==ev.getElecTriggerSector(false);
//    	if(!good_trig) return false;
...
//				System.out.println("PROT: "+protECAL.get(0).getProperty("x")+" "+protECAL.get(0).getProperty("y"));
	...
	public void fillECkin() {
		...
		DataGroup dg6 = this.getDataGroup().getItem(0,6,k,run);	
...
//		((H2F) ECkin.getData(0).get(0)).fill(-e_x,e_y);
	...
	public void fillECelec() {
		...
		int kpim = getDetectorTabNames().indexOf("ECpim");
...
//		DataGroup dg06 = this.getDataGroup().getItem(0,6,k1,run);
		...
		DataGroup dgpim = this.getDataGroup().getItem(0,4,kpim,run);
		((H2F) dgpim.getData(e_sect-1+ 6).get(0)).fill(e_the,e_mom);
		((H2F) dgpim.getData(e_sect-1   ).get(0)).fill(e_the,e_mom);
...
//			}
		...
		// Radiated photon e-gamma theta,phi residuals
    	...
    	for (Map.Entry<Long,List<Particle>>  entry : ecphot_filt.getMap().entrySet()){
			int is = ig.getIndex(entry.getKey(), 0);
            	for (Particle ec : entry.getValue()) {				
            		...
            		float    thdif = (float)(e_the-ecal_the)*Math.signum(ev.tpol);  //tpol: torus polarity          		
                    float    phdif = (float)(ephi-ecalphi)  *Math.signum(ev.spol);  //spol: solenoid polarity
	...
	public void fillECtime(int id) {
		...
		for (Particle pl : plist){
	        ...
	        if(p_beta_pcal>0 || p_beta_ecin>0 || p_beta_ecou>0 || p_beta_ftof>0) {
...
//			System.out.println(nE+" "+is+" "+ev.trigger_sect);
			...
			DataGroup dg4 = this.getDataGroup().getItem(0,4,k,run);
			((H2F) dg4.getData(e_sect-1+12).get(0)).fill(the,mom);
			((H2F) dg4.getData(e_sect-1   ).get(0)).fill(the,mom);
...
//						System.out.println(bits.toString()+"\t"+lay+" "+(-x)+" "+p.getProperty("x")+" "+y+" "+p.getProperty("y")+" "+z+" "+p.getProperty("z"));
    ...
    public void fillECnm2(String name) {
        ...
        NeutralMeson nm = new NeutralMeson(ecphot); nm.setThresh(3.0);
    ...
    public void fillECnm3(String name) {
        ...
        NeutralMeson nm = new NeutralMeson(ecphot); nm.setThresh(1.0);
...
//        DataGroup ECpip2  = this.getDataGroup().getItem(0,2,getDetectorTabNames().indexOf("ECpip"),run);
        ...
        DataGroup ECneut2 = this.getDataGroup().getItem(0,2,getDetectorTabNames().indexOf("ECneut"),run);
...
//          if(pip_lay==7) {((H1F) ECpip2.getData(pip_sec-1+12).get(0)).fill(pip_nrg); pvec[2]=new Vector3(x,y,z);}		  
        ...
        for (Particle p: neut_ecal) {
          ...
          for (Particle pp : neutECAL) {
            ...
            if (layer==1) ((H1F)ECneut2.getData(0).get(0)).fill((float) pp.getProperty("time")-ev.starttime);
            if (layer==4) ((H1F)ECneut2.getData(1).get(0)).fill((float) pp.getProperty("time")-ev.starttime);
            if (layer==7) ((H1F)ECneut2.getData(2).get(0)).fill((float) pp.getProperty("time")-ev.starttime);
          }
...
*/
    ...
    public void fillECneut2() {
    	int run = getRunNumber();
        DataGroup ECneut2 = this.getDataGroup().getItem(0,2,getDetectorTabNames().indexOf("ECneut"),run);
        
        for (Particle p : neut2_ecal) {
            List<Particle> neutECAL2 = ev.getECAL((int)p.getProperty("pindex"));            
            ecal_neut_sec  = (int)   neutECAL2.get(0).getProperty("sector");
            ecal_neut_beta = (float) neutECAL2.get(0).getProperty("beta");            
            ecal_neut_the  = (float) Math.toDegrees(p.theta());
            ecal_neut_phi  = (float) Math.toDegrees(p.phi());
            if (ecal_neut_beta<0.9) {
            for (Particle pp : neutECAL2) {
              int layer = (int) pp.getProperty("layer");
              float x = (float) pp.getProperty("time")-ev.starttime, y = (float) pp.getProperty("energy"); 
              if (layer>0) ((H2F)ECneut2.getData(layer/3).get(0)).fill(x,y);
            }  
            }
        }
