		...
		public void run() {
			...
			for (int i = 0; i < fitFunction.getNPars(); i++) {
				if (i == 0) {					
					...
					fitFunction.setParLimits(1,currentRangeMin, currentRangeMax);
				} else if (i == 2) {
					double sig = getRMSIDataSet(currentDataset, currentRangeMin, currentRangeMax);
					fitFunction.setParameter(2, sig);
					fitFunction.setParLimits(2, 0.0, sig*5);
				} else if (i == 3) {
